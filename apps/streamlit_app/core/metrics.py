...
def calc_pr_metrics(pred: np.ndarray, target: np.ndarray) -> Dict[str, Any]:
    ...
    return {"tp": tp, "fp": fp, "fn": fn, "tn": tn, "precision": precision, "recall": recall, "iou": iou}


def aggregate_verification(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    has_gt = [r for r in rows if r.get("coverage_gt") is not None]
    if not has_gt:
        return {"avg_gt": None, "avg_delta": None, "precision": None, "recall": None, "f1": None, "iou": None}
    ...
    iou = tp / (tp + fp + fn) if (tp + fp + fn) > 0 else None
    return {"avg_gt": avg_gt, "avg_delta": avg_delta, "precision": precision, "recall": recall, "f1": f1, "iou": iou}
